...
La subdivisión del dataset tiene el siguiente objetivo: Sobre el conjunto de ‘train’ se prueba inicialmente los métodos de segmentación y se busca un conjunto de  hiperparametros que permita obtener la mejor segmentación posible. El conjunto de 'validation' existe para validar el modelo de aprendizaje y los hiper-paramentos encontrados en set de ‘train’ de tal manera que se tengan resultados aceptables dentro de un conjunto de imágenes no visto antes. Cuando se encuentra un modelo e hiperparametros que presentan un buen desempeño tanto en el conjunto de 'train' como el de 'validation', se usa el conjunto de 'test' para establecer la calidad del método (precisión, exhaustividad y F-Medida en este caso).

Ninguna de las estrategias de segementación utilizadas en este laboratorio requiere de entrenamiento o ajuste de hiperparamteros (mas alla del valor de K, que ya fue discutido). Debido a esto se utiliza unicamente el conjunto de 'test' y se evaluan direcatmente las segmentaciones según la verdad de terreno.
...
Las curvas obtenidas para los métodos propuestos en este laboratorio comparten la misma deficiencia:  no pueden obtener en ningún caso la precisión de 1.0 (en general nunca es mejor de 0.5) ni la exhaustividad de 1.0 (se acerca a la frontera de 0.9). Esto indica que en ninguna de las combinaciones probadas dentro de este laboratorio (espacio de color y K) se logró un número cercano a cero de falsos positivos o falsos negativos. El desempeño de los dos métodos propuesto es bastante similar, sin embargo, se puede afirmar que la precision del metodo LAB+xy GMM es siempre superior a la del método HSV+xy GMM, a costa de un menor valor de precisión, es decir la segmentación de HSV+xy GMM contiene menos falsos positivos pero a su vez contiene más falsos negativos.


\subsection{Análisis de resultados}

Existe una clara diferencia en desempeño entre los algoritmos creados para el laboratorio y el UCM. La diferencia se puede explicar en gran medida en que los algoritmos utilizan únicamente la información local de color del píxel y su localización absoluta en la imagen. Este tipo de información permite aproximarse a los verdaderos contornos y tiene varias condiciones de falla entre otras:
...
\section{Crédito Extra}
recibidas después de la entrega) de la BSDS-500. Se utilizan los algoritmos de clustering: GMM, Hierarchical cluster y KMeans, y se corren únicamente el espacio de color HSV+xy. Adicionalmente se utiliza un K entre 2 y 15 para mejorar un poco los resultados de la segmentación.


\begin{table*}[th!]
\centering
\begin{tabular}{c | c | c | c }
Método & ODS & OID & AreaPR  \\
\hline	
HSV+xy GMM & F( 0.78, 0.40 ) = 0.53  & F( 0.80, 0.47 ) = 0.59 & 0.30 \\
HSV+xy KMeans &  F( 0.71, 0.38 ) = 0.49 & F( 0.75, 0.43 ) = 0.54 &  0.28\\
HSV+xy Hierarchical & F( 0.66, 0.47 ) = 0.55  & F( 0.71, 0.51 ) = 0.59 &  0.33\\
\end{tabular}
\label{table:thetable2}
\end{table*}

En la tabla \ref{table:thetable2} se puede observar que el desempeño de los métodos mejora ligeramente respecto a lo presentado originalmente en la tabla \ref{table:thetable}, este resultado se debe principalmente a que se tiene 2 clusters extra (K va hasta 15) que posiblemente mejoran el desempeño en imágenes que tienen más de 13 objetos.


Al igual que en el experimento inicial, ninguna de la curvas alcanza la precisión de 1, tampoco se logra llegar a una cobertura de 1. el mejor caso se presenta con el método de hierarchical clustering, con una precisión máxima por debajo de 0.6 y una cobertura apenas superior a 0.8. En ningún caso se solucionan las deficiencias de los métodos utilizados y, por tanto, la diferencia entre los métodos propuestos y la estrategia de UCM se mantiene.

Una posible estrategia para mejorar los resultados sería mezclar la salida de los algoritmos de GMM y Hierarchical clustering, bajo esta estrategia es posible que se mejore la cobertura del segundo método.
...
\includegraphics[width=1.15\linewidth]
                {img/UCM2.jpg}
\end{center}
\caption{Curvas de precision y recall para los métodos usados en el credito extra. En verde la curva de hierachical clustering, en negro la de GMM y en rojo la curva de k-Means}
\label{fig:plot2}
\end{figure}
