...
function my_segmentation = segment_by_clustering( img,feature_space,numclusters)
    scaleFactor=5
    ...
    clusterKmeans=KmeansSegment(lineImage,originalSize,numclusters);
    clusterGMM=GMMSegment(lineImage,originalSize,numclusters);
    clusterHierachy=HierachicalSegment(lineImageResize,resizedSize,numclusters);
    
    clusterHierachyResized=imresize(clusterHierachy,[originalSize(1),originalSize(2)]);

    %imshow(horzcat(mat2gray(rgb2gray(img)),mat2gray(clusterKmeans),mat2gray(clusterGMM),mat2gray(clusterHierachyResized)));
    im=imagesc( horzcat(clusterKmeans,clusterKmeans,clusterGMM,clusterHierachyResized));
